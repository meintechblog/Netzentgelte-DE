    ...
    await db
      ...
      .values(
        rows.map((row) => ({
          ...
          registryFeedSource: row.registryFeedSource,
          registryFeedLabel: row.registryFeedLabel,
          lastSeenInRegistryFeed: row.lastSeenInRegistryFeed,
          deprecatedStatus: row.deprecatedStatus,
          deprecatedCheckedAt: row.deprecatedCheckedAt,
      ...
      .onConflictDoUpdate({
        ...
        set: {
          ...
          registryFeedSource: sql.raw(`excluded.${operatorShells.registryFeedSource.name}`),
          registryFeedLabel: sql.raw(`excluded.${operatorShells.registryFeedLabel.name}`),
          lastSeenInRegistryFeed: sql.raw(`excluded.${operatorShells.lastSeenInRegistryFeed.name}`),
          deprecatedStatus: sql.raw(`excluded.${operatorShells.deprecatedStatus.name}`),
          deprecatedCheckedAt: sql.raw(`excluded.${operatorShells.deprecatedCheckedAt.name}`),
          deprecatedReason: sql.raw(`excluded.${operatorShells.deprecatedReason.name}`),
