...
describe("buildAutomationCommandPlan", () => {
  test("assembles the verification, deploy and live-check commands in the required order", () => {
    ...
    expect(plan.deploy).toEqual([
      "rsync -az --delete --exclude='.git' --exclude='node_modules' --exclude='.next' --exclude='.playwright-cli' --exclude='data/artifacts' --exclude='tmp' /Users/hulki/projects/netzentgelte-de/ root@192.168.3.178:/root/netzentgelte-de/",
